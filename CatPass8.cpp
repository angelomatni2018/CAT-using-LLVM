...
namespace {
  ...
  struct CAT : public ModulePass {
    ...
    bool analyzeLoopsInFunction(Function &F) {
      ...
      for (auto i : LI){
        auto loop = &*i;
        changed |= analyzeLoop(LI, loop, DT, SE, AC, ORE);
      }
      return changed;
    }

    bool analyzeLoop(LoopInfo &LI, Loop *loop, 
      ...
      if (loop->isLoopSimplifyForm() && loop->isLCSSAForm(DT)) {
        ...
        auto unrolled = UnrollLoop(loop, 4, tripCount,
            forceUnroll, allowRuntime, allowExpensiveTripCount,
            preserveCondBr, preserveOnlyFirst, tripMultiple, 0, 
            &LI, &SE, &DT, &AC, &ORE, true);
        ...
        for (auto elem : dfa->in[call]) {
          ...
          if (auto* reachingCall = dyn_cast<CallInst>(reachingInst)) {
            ...
            if (isCATUpdate(callee)) {
              ...
              if (def == getArg || isInAlias(dfa->mayAliases, getArg, def)) {
                updateReaches = true;
                break;
              }
            }
